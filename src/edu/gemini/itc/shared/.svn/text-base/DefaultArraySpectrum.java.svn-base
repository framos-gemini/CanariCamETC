...
//
// $Id: DefaultArraySpectrum.java,v 1.6 2004/02/13 13:00:59 bwalls Exp $
//
...
public class DefaultArraySpectrum implements ArraySpectrum {
     ...
     * @throws If lists do not have same length or if they contain
     ...
     * data[0][i] = x values
     * data[1][i] = y values
     * @throws If array is not of proper form Exception is thrown
     */
    ...
    public double[][] getData(int maxIndex) {
        if (maxIndex >= _data[0].length) maxIndex = _data[0].length;
        double[][] data = new double[2][maxIndex + 1];
